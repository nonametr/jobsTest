...
[ExecuteInEditMode]
public class StreamTerrainGenerator : MonoBehaviour
{
	...
	{
		public static StreamTerrainGenerator TerrainGenerator;
		[MenuItem("MMO/StreamTerrain splatmap preview")]
...
#if UNITY_EDITOR
[CustomEditor(typeof(StreamTerrainGenerator))]
public class StreamTerrainGeneratorEditor : Editor
{
	public override void OnInspectorGUI()
	{
		DrawDefaultInspector();

		StreamTerrainGenerator obj = (StreamTerrainGenerator) target;
		...
		using (new GUILayout.VerticalScope(GUI.skin.box))
		{
			...
			if(obj.showControlMapPreview)
			{
				...
				{
					StreamTerrainGenerator.SplatmapPreviewWindow.TerrainGenerator = obj;
					StreamTerrainGenerator.SplatmapPreviewWindow.ShowWindow();
				}
				else
				{
			...
			for (int i = 0; i < obj.splatMapEntries.Count; ++i)
			{
				using (new GUILayout.VerticalScope(GUI.skin.box))
				{
					...
					obj.splatMapEntries[i].channel = (StreamTerrainGenerator.SplatMapEntry.TextureChannel) EditorGUILayout.EnumPopup(obj.splatMapEntries[i].channel, GUILayout.Width(64));
					EditorGUILayout.LabelField(new GUIContent("Transition:"), GUILayout.Width(64));
					obj.splatMapEntries[i].transition = (StreamTerrainGenerator.SplatMapEntry.Transition) EditorGUILayout.EnumPopup(obj.splatMapEntries[i].transition, GUILayout.Width(64));
					EditorGUILayout.EndHorizontal();

					EditorGUILayout.BeginVertical();
					EditorGUILayout.BeginHorizontal();
					obj.splatMapEntries[i].diffuse_tex = (Texture2D) EditorGUILayout.ObjectField(obj.splatMapEntries[i].diffuse_tex, typeof(Texture2D), false, GUILayout.Width(64), GUILayout.Height(64));
					obj.splatMapEntries[i].normal_tex = (Texture2D) EditorGUILayout.ObjectField(obj.splatMapEntries[i].normal_tex, typeof(Texture2D), false, GUILayout.Width(64), GUILayout.Height(64));
					EditorGUILayout.EndHorizontal();
			...
			{
				StreamTerrainGenerator.SplatMapEntry entry = new StreamTerrainGenerator.SplatMapEntry();
				obj.splatMapEntries.Add(entry);
				obj.SplatMapControlRT.material.shader = Shader.Find(obj.SplatMapControlRT.material.shader.name);
			}
		}
		...
		if (EditorGUI.EndChangeCheck())
		{
			if (obj.splatMapEntries.Count > 0)
			{
				...
				obj.SplatMapControlRT.material.SetVector("_RenderArea", new Vector4(0, 0, 1, 1));
